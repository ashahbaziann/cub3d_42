...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/03/30 17:06:19 by ashahbaz          #+#    #+#             */
/*   Updated: 2025/05/08 17:38:10 by ashahbaz         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	perform_dda(t_game *game, t_ray *ray)
{
	...
	while (!ray->hit)
	{
		...
		if (ray->map_y < 0.25 || ray->map_x < 0.25 || ray->map_y > game->height
			- 0.25 || ray->map_x > game->width - 1.25)
			break ;
...
{
	if (ray->side == 0)
		ray->wall_dist = (ray->map_x - game->player.x + (1 - ray->step_x) / 2)
			/ ray->dir_x;
	else
		ray->wall_dist = (ray->map_y - game->player.y + (1 - ray->step_y) / 2)
			/ ray->dir_y;
...
void	raycast(t_game *game)
{
	...
	game->img.address = mlx_get_data_addr(game->img.img, &game->img.bpp,
